...
- a **confidence** tag — `[multi-run]` (held across all timing iterations / reproduced by ≥2 separate checks; **note:** timing `[multi-run]` here means **multiple in-process iterations within a single process run**, not repeated separate process launches — no `*_run2` cross-process re-run was captured, see Gaps), `[single-observation]` (measured once), or `[hypothesis]` (mechanism proposed but no attribution experiment run); and
...
**Consequence for the writer:** most findings here are **reproductions + quantification + live re-confirmation of already-public issues**, and must be framed that way (with the issue links), not as discoveries. The third-party benchmark corroboration (heading=0, table fidelity low) is a *strength* of the evidence, not a weakness — it means our numbers agree with an outside source. The one finding that clears all three novelty arms is the **~3.35 s cold-import tax** (FINDING-02).
...
|---|---:|:--:|---:|---:|:--:|---:|---:|
| books.toscrape | 10,478 | 1 / 0 / 0 | 0 | 94 | 5/5 | **0.6%** (1/159) | 84 |
| quotes.toscrape | 2,973 | 1 / 1 / 0 | 0 | 55 | 4/4 | **1.2%** (1/86) | 48 |
| scrapethissite/forms | 3,385 | 1 / 0 / 0 | **27** | 31 | 4/4 | **6.7%** (5/75) | 93 |
| wikipedia/Web_scraping | 60,159 | 1 / **7** / **12** | 9 | 418 | 5/5 | **12.4%** (42/338) | 352 |

**FINDING-04 [multi-run] [DOCUMENTED]:** on well-tagged HTML, MarkItDown's HTML→Markdown is **fast and content-complete** — all four pages convert in **~48–352 ms** on local fixtures (median-of-5; the 226 KB Wikipedia page is the slow end at 352 ms, the small quotes page the fast end at 48 ms), every body probe survives (5/5, 4/4, 4/4, 5/5), heading structure mirrors the source (Wikipedia: h1×1/h2×7/h3×12, matching the article's section tree), and links are preserved as real `[text](url)` (418 on Wikipedia). The 26×9 hockey-stats `<table>` on the forms page becomes a proper GFM pipe table (27 rows = header + separator + 26 data rows) with empty cells preserved. This is the whole-document conversion the tool is built for, and on clean HTML it is faithful.
...
|---|---:|---:|---:|:--:|---:|---|
| arXiv 1706.03762 (text-layer PDF) | 2.2 MB | 40,174 | 72 | **7/7** | 3,707 | title, "Transformer", "Multi-Head Attention", "BLEU", "References" all present |
| Bitcoin whitepaper (9 pp PDF) | 184 KB | 22,485 | 24 | **6/6** | 1,391 | "Satoshi Nakamoto", "Abstract", "proof-of-work", "Conclusion" present |
| **scanned PDF (no text layer)** | 89 KB | **0** | 0 | **0/4** | 15 | **empty output, no error, no OCR** — FINDING-05 |
| DOCX (test.docx) | 136 KB | 4,651 | 7 | — | 70 | headings + GFM table + image alt-text; embedded UUIDs survive |
| DOCX equations | 15 KB | 240 | 0 | — | 101 | **LaTeX math preserved**: `$$sin θ=\frac{mλ}{a}…$$` — FINDING-10 |
| XLSX (test.xlsx) | 12 KB | 808 | 31 | — | 57 | each sheet → `## SheetName` + GFM table |
| PPTX (test.pptx) | 278 KB | 2,047 | 11 | — | 52 | `<!-- Slide number: N -->` markers, tables, even a chart→table |

*Timing note (deep_convert vs scale bench):* the arXiv median here is **~3.7 s**, run under `deep_convert.py`'s `run_docs` (n=3, warm process, whole-suite run). §E re-runs the same arXiv PDF **in its own isolated process** under `scale_bench.py` (n=3, cold process baseline) and measures **~12.6 s** — see [FINDING-11](#e-scale--stress). The gap is the harness difference, not a contradiction: `scale_bench.py` pays a cold-process cost and isolates RSS, while `deep_convert.py` measures a warm in-suite convert. For a single transferable number on this PDF, prefer §E's isolated-process 12.6 s; the 3.7 s here is the warm-suite figure. (The pdfplumber per-page form-detection cost dominates both — see FINDING-11.)

**FINDING-05 [multi-run] [KNOWN-ISSUE: [#1268](https://github.com/microsoft/markitdown/issues/1268)]:** on an **image-only PDF with no text layer**, MarkItDown returns an **empty string** — **0 characters, no exception, no warning** (converted in ~15 ms because there is nothing to extract). MarkItDown's PDF path is text-extraction only (pdfminer/pdfplumber); it ships **no OCR** in core or any pip extra. A developer feeding a batch of PDFs where some are scans gets silently empty results for those files with no signal that anything was skipped. This reproduces the long-open OCR-fallback gap (#1268 and siblings) with the exact no-error behavior. The recommended path (per the tracker and README) is the optional Azure Document Intelligence backend or a plugin — neither is in the default install. *Root-cause check:* a separate probe going straight to the same extractor — `pdfminer.six` `high_level.extract_text` on the fixture — recovers **0 stripped characters** (`stripped_chars=0`, `has_text_layer=false`), confirming the fixture has no text layer, so the empty output is MarkItDown's behavior on a real scan, not a broken fixture. Probe: [pdfminer_textlayer_probe.py](tests/pdfminer_textlayer_probe.py) → [textlayer_probe.json](artifacts/raw/textlayer_probe.json).

**FINDING-06 [multi-run] [DOCUMENTED]:** across **both** text-layer PDFs, MarkItDown produces **zero Markdown heading markers** (0/0/0 on the arXiv paper and the Bitcoin whitepaper) — a PDF carries no semantic heading tags, and MarkItDown does not infer them from font size, so every line lands at body level. Text recall is high (7/7 and 6/6 probes), but the *structure* is flat. This **reproduces** third-party public benchmarks that score MarkItDown's PDF heading-hierarchy at ~0.0 and table fidelity ~0.27, far below Docling's TableFormer-backed ~0.88 ([danilchenko.dev](https://www.danilchenko.dev/posts/markitdown-vs-docling-vs-marker/), [READoc](https://arxiv.org/pdf/2409.05137)). The trade-off those benchmarks also report — MarkItDown is ~**100× faster** than Docling — is consistent with our seconds-not-minutes times (Bitcoin whitepaper ~1.4 s, arXiv paper ~3.7 s warm / ~12.6 s in an isolated process) on documents that take a layout-model tool like Docling minutes. (This is a published-benchmark comparison, not a same-fixture head-to-head — see Gaps.) Suggested article framing: *MarkItDown gives you clean, fast PDF **text**; it does not give you the PDF's **structure**. If headings/tables must survive, a layout-model tool (Docling/Marker) is the right layer.*

**FINDING-10 [single-observation] [EXCLUSIVE — minor]:** the DOCX path (via `mammoth`) **preserves Office Math (OMML) equations as LaTeX** — `equations.docx` converts `$$\frac{a}{λ}=\frac{m}{sin θ}=…$$` and inline `$θ=sin^{-1}(2.2×10^{-4})$`. Issue/SERP search surfaced no write-up of this specific behavior; it is a real, if narrow, strength for anyone converting math-heavy Word docs for an LLM. (DOCX/XLSX/PPTX all also kept every embedded UUID sentinel, i.e. **no silent content loss** on the maintainers' own regression fixtures.)
...
**FINDING-02 [single-observation] [EXCLUSIVE]:** even after a clean install, **`import markitdown` costs ~3.35 s** on this machine (single `python -X importtime` profile). The cost is almost entirely at import time: `markitdown._markitdown` eagerly imports the **entire converter registry** (`markitdown.converters`, **2.56 s cumulative** — 76% of the total), which pulls **pandas (594 ms, via the XLSX converter)**, **python-pptx (427 ms)**, **magika (354 ms)**, and **requests (270 ms)** — whether or not you convert any of those formats. All figures are the cumulative import-time (µs→ms) columns from `python -X importtime`; raw in [importtime.txt](artifacts/logs/importtime.txt) (`markitdown` 3,352,795 µs; `markitdown.converters` 2,564,010 µs; `pandas.core.api` 594,431; `pptx` 426,988; `magika` 354,018; `requests` 269,995). This is a single profiled run — treated as one observation, not a multi-run distribution (no per-process cold-import distribution was captured; see Gaps). Issue-tracker + SERP search for this returned **zero MarkItDown-specific hits** (only generic "pandas import is slow" threads). For a long-running service the import cost is amortized, but for a CLI invocation or a serverless/Lambda cold start it is a real per-process tax that a "lightweight utility" framing does not lead you to expect.

**FINDING-03 [single-observation] [KNOWN-ISSUE (magika-optional): [#1234](https://github.com/microsoft/markitdown/issues/1234)]:** the **core** install is **161 MB** (empty venv 13 MB → +148 MB), of which **onnxruntime (73 MB) + numpy (34 MB) = 107 MB, i.e. 66% of the entire core footprint**, are dragged in by a single hard dependency: **`magika`** (Google's ML file-type detector, 3.3 MB itself). So a *text converter* ships a **73 MB ONNX inference runtime** in its base install before any document extra is added. With document extras (pandas, lxml, pdfminer/pdfplumber, Pillow, python-pptx, mammoth) the venv reaches **310 MB**. Making magika optional is an open request (#1234). This is much lighter than a browser-download stack, but readers expecting a `pip install`-and-done micro-utility should know an ONNX runtime rides along. Freeze: [pip-freeze.txt](artifacts/logs/pip-freeze.txt).

## Provisional scorecard

**Provisional**, scored **through the lens of a document/HTML→Markdown converter** (crawl/JS dimensions are scored low because the tool simply does not do them — which matters for anyone shopping for a "scraper"). Weights mirror the rest of the series for cross-tool comparability; this is not a final article rating and should be reweighted for a "converter" category before publishing.

| Dimension | Weight | Score | Evidence |
|---|---:|---:|---|
| Setup and first run | 10 | **5** | core+HTML works instantly, but `[all]`→0.0.2 (FINDING-01); 161 MB core w/ 73 MB onnxruntime (FINDING-03); ~3.35 s cold import (FINDING-02) |
...
- Frame MarkItDown accurately as **"the last mile" of a scraping pipeline** — the thing that turns fetched HTML/PDF/Office files into LLM-ready Markdown — not a scraper competing on fetching.
- Positive evidence (highest-scoring dimensions): **full body recall + clean HTML/Office table→GFM on well-formed input**, **PDF/DOCX text with high recall** (arXiv 7/7 probes in ~3.7 s warm / ~12.6 s isolated; DOCX equations→LaTeX), and **no silent content loss** on the maintainers' own fixtures.
- Must-tell caveats, each with a number: **keeps boilerplate** (Wikipedia 12.4% chrome lines); **tables break on spans/nesting/in-cell pipes** (2/13 broken, 4/13 ragged, and the rowspan **column-shift** in FINDING-07a); **scanned PDF → silently empty, no OCR** (FINDING-05); **PDF has zero heading structure** (FINDING-06, and it agrees with public benchmarks); the **`[all]`→0.0.2** install trap (FINDING-01) and the **~3.35 s cold import** (FINDING-02).
- The install gotcha is useful, practical content: **don't `pip install markitdown[all]`** — pin the version + install extras individually.
...
- **Long-batch memory growth / leaks / GIL and process lifecycle not tested.** All timings are single-process (N in-process iterations); there is **no separate-process re-run** (`*_run2`) and **no long loop measuring RSS drift across hundreds of converts**. So we cannot say whether repeated `convert()` calls leak, whether peak RSS grows over a batch, or how the (CPU-bound) PDF path behaves under a multi-thread GIL-contended workload. For a tool positioned as the last-mile converter in a batch pipeline, a 100+-document loop with a cold baseline and an RSS curve (in its own process) is the missing measurement.
...
- Third-party benchmarks: [MarkItDown vs Docling vs Marker — danilchenko.dev](https://www.danilchenko.dev/posts/markitdown-vs-docling-vs-marker/), [PDF-to-Markdown deep dive — jimmysong.io](https://jimmysong.io/blog/pdf-to-markdown-open-source-deep-dive/), [READoc benchmark (arXiv 2409.05137)](https://arxiv.org/pdf/2409.05137)
