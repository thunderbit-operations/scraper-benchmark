#!/usr/bin/env python3
"""H5 — concurrency model cost: shared browser (child contexts/tabs) vs separate
browsers.

N=4 navigations, done two ways: (shared) one browser with N child contexts, and
(separate) N independent browsers. Reports wall-time distribution over >=3 runs and
the peak Chrome *browser*-process count for each. RUN ALONE (timing-sensitive).
...
def run_mode(base: str, mode: str) -> list[dict[str, Any]]:
    rows = []
    for r in range(REPS):
        key = os.path.join(tempfile.gettempdir(), f"chromedp_conc_{mode}_{int(time.time()*1000)}_{r}")
    ...
    try:
        ...
        summary = {
            ...
            "verdict": {
                ...
                "note": "Shared-browser tabs run N navigations in ONE Chrome process; separate "
