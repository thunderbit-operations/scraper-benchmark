...
# --- Archival-specific ADDITIVE routes (Heritrix pack; not in katana ground truth) ---
# Two DISTINCT URLs that return BYTE-IDENTICAL content -> content-digest dedup probe.
# A crawler that dedups on content digest should emit a WARC `revisit` record for the
# second URL instead of a second full `response` payload.
DUP_ENDPOINTS = ["/dup/one", "/dup/two"]
DUP_PAYLOAD = ("<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\">"
               "<title>Duplicate Content Body</title></head><body>"
               "<h1>Identical Archival Body</h1>"
               "<p>This exact byte sequence is served at two different URLs so a "
               "content-digest dedup can be measured against ground truth.</p>"
               "</body></html>")
# A path linked from the home page but DISALLOWED in robots.txt -> obedience probe.
# Server-side hit counter on this path proves whether the fetch was suppressed.
ROBOTS_DENIED_ENDPOINT = "/robots-denied/secret"
...
def ground_truth(base_url: str) -> dict[str, Any]:
    return {
        ...
        "dup_identical_content": DUP_ENDPOINTS,
        "robots_denied": ROBOTS_DENIED_ENDPOINT,
    }
...
class FixtureHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/":
            ...
            arch_links = (f'<a href="{DUP_ENDPOINTS[0]}">dup one</a>'
                          f'<a href="{DUP_ENDPOINTS[1]}">dup two</a>'
                          f'<a href="{ROBOTS_DENIED_ENDPOINT}">robots-denied</a>')
...
<div id="archival">{arch_links}</div>
...
{RUNTIME_DOM_SCRIPT}"""
        ...
        # Byte-identical content at two distinct URLs (content-digest dedup probe).
        if path in DUP_ENDPOINTS:
            self._send(HTTPStatus.OK, DUP_PAYLOAD.encode("utf-8"))
            return

        # robots-Disallowed path: if Heritrix obeys robots, this counter stays 0.
        if path.startswith("/robots-denied/"):
            self._send(HTTPStatus.OK, html_page("Secret", "<h1>should be robots-blocked</h1>"))
            return
        ...
        if path == "/robots.txt":
            ...
            # Disallow the robots-denied path (obedience probe); everything else is
            # allowed by default. Sitemap directive drives known-file discovery.
            body = (f"User-agent: *\n"
                    f"Disallow: /robots-denied/\n"
