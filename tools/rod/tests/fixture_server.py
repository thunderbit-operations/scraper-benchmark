#!/usr/bin/env python3
"""Local fixture server for the rod evidence pack.

Byte-for-byte the SAME three content classes as the chromedp / playwright-mcp / katana
fixtures, so rod's recall numbers are directly comparable to chromedp's on the same page.
The point is THREE content classes that separate *when* content exists in the DOM, so a
live-browser driver's wait/idiom is what decides recall (not merely "it's a browser").
...
presence/absence against a known set — never guesses):
  ...
  C. Delayed-injected — a node created `DELAY` ms AFTER the load event via
     ...
     navigate MISSES class C; only an auto-waiting query (rod's Element) or a poll
     recovers it. This is the adversarial timing boundary, and the SAME content class
     katana's static crawl misses, playwright-mcp catches, and chromedp catches only
     with a node-keyed WaitVisible.

Plus: a /waitsem page (attached-but-hidden node for Element-vs-WaitVisible), leaf
pages, a 500 route and a dead link. A server-side hit counter records which paths
Chrome actually FETCHED — fetch-truth independent of rod's own return values.
"""
...
# rod reports. This is how class-A/B/C leaf fetches and robustness are proven.
...
def delayed_script(delay_ms: int) -> str:
    ...
    # or near load) cannot see it without an auto-waiting query or a poll. Marker +
    # href assembled from fragments so only executing + waiting reveals them.
    return f"""
<script>
...
class FixtureHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        # visible node, both with real ids, for Element vs WaitVisible.
        if path == "/waitsem":
            body = (
...
def start_fixture_server(port: int | None = None) -> FixtureServer:
    ...
    thread = threading.Thread(target=httpd.serve_forever, name="rod-fixture", daemon=True)
    thread.start()
