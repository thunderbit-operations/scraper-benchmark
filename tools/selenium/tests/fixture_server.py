#!/usr/bin/env python3
"""Local fixture server for the selenium evidence pack.

Byte-for-byte the SAME three content classes as the chromedp / rod / playwright-mcp / katana
fixtures, so selenium's recall numbers are directly comparable to the CDP drivers' on the
same page. The point is THREE content classes that separate *when* content exists in the
DOM, so a live-browser driver's wait/idiom is what decides recall (not merely "it's a
browser"). Selenium reaches this page through chromedriver (W3C WebDriver), not CDP — the
axis this pack probes.
...
presence/absence against a known set — never guesses):
  ...
  C. Delayed-injected — a node created `DELAY` ms AFTER the load event via
     setTimeout. Marker + href assembled from fragments too. A naive read after
     navigate MISSES class C; only an explicit wait (Selenium's WebDriverWait +
     expected_conditions) or a poll recovers it. This is the adversarial timing
     boundary, and the SAME content class katana's static crawl misses, playwright-mcp
     catches, chromedp catches only with a node-keyed WaitVisible, and rod catches with
     its auto-waiting Element().

Plus: a /waitsem page (attached-but-hidden node for presence-vs-visibility), leaf
pages, a 500 route and a dead link. A server-side hit counter records which paths
Chrome actually FETCHED — fetch-truth independent of selenium's own return values.
"""
...
# selenium reports. This is how class-A/B/C leaf fetches and robustness are proven.
...
def delayed_script(delay_ms: int) -> str:
    # Class C: inject DELAY ms AFTER the load event, so a naive read (which happens at
    # or near load) cannot see it without an explicit wait or a poll. Marker + href
    # assembled from fragments so only executing + waiting reveals them.
    return f"""
<script>
...
class FixtureHandler(BaseHTTPRequestHandler):
    server_version = "SeleniumFixture/1.0"
    ...
    def do_GET(self) -> None:
        ...
        # visible node, both with real ids, for presence vs visibility.
        if path == "/waitsem":
            body = (
...
def start_fixture_server(port: int | None = None) -> FixtureServer:
    ...
    thread = threading.Thread(target=httpd.serve_forever, name="selenium-fixture", daemon=True)
    thread.start()
