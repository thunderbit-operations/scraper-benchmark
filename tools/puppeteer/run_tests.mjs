#!/usr/bin/env node
// Reproducible evaluation material for the Puppeteer single-tool pack.
// Puppeteer is a Chrome/Chromium browser-automation library (Node).
// JS content renders natively; no built-in crawl queue. Research harness, not blog copy.
...
async function main() {
  ...
  const summary = {
    run_started_at: now(), tool: 'puppeteer',
  ...
  try {
    // 1) Static catalog + pagination.
    {
      ...
      while (url) {
        ...
        const cards = await page.$$eval('.product-card', (els) => els.map((el) => ({ id: Number(el.getAttribute('data-product-id')), name: el.querySelector('.product-name')?.textContent?.trim(), category: el.querySelector('.category')?.textContent?.trim(), price: Number(el.querySelector('.price')?.textContent?.replace('$', '').trim()), rating: Number(el.querySelector('.rating')?.textContent?.replace(' stars', '').trim()) })));
        rows.push(...cards);
        const nextHref = await page.$eval('.next-page', (a) => a.getAttribute('href')).catch(() => null);
        url = nextHref ? new URL(nextHref, base).href : null;
      }
      await page.close();
    ...
    // 2) Article.
    {
      const t0 = Date.now(); const page = await browser.newPage();
    ...
    // 3) Dynamic JS page renders natively + screenshot.
    {
      const t0 = Date.now(); const page = await browser.newPage();
      ...
      await page.screenshot({ path: path.join(SHOTS_DIR, 'local_dynamic_puppeteer.png'), fullPage: true });
      await page.close();
      rows.sort((a, b) => a.id - b.id);
      writeJson('local_dynamic_rendered.json', { status_text: statusText, products: rows });
      summary.tests.local_dynamic_rendered = { url: `${base}/dynamic/catalog`, success: rows.length > 0, items: rows.length, status_text: statusText, product_recall: recall(rows, DYNAMIC_PRODUCTS.map((p) => p.name)), screenshot: 'results/screenshots/local_dynamic_puppeteer.png', elapsed_seconds: Number(((Date.now() - t0) / 1000).toFixed(3)) };
    }

    // 4) Dynamic JSON API via in-page fetch.
    {
      const t0 = Date.now(); const page = await browser.newPage();
      await page.goto(`${base}/`, { waitUntil: 'domcontentloaded' });
      const rows = await page.evaluate(async () => (await (await fetch('/api/dynamic-products')).json()));
      await page.close();
      writeJson('local_dynamic_api.json', rows); writeCsv('local_dynamic_api.csv', rows);
      summary.tests.local_dynamic_api = { url: `${base}/api/dynamic-products`, success: true, items: rows.length, product_recall: recall(rows, DYNAMIC_PRODUCTS.map((p) => p.name)), note: 'In-page fetch reaches the API without scraping the DOM.', elapsed_seconds: Number(((Date.now() - t0) / 1000).toFixed(3)) };
    }
    ...
    // 5) HTTP 500.
    {
      ...
      summary.tests.local_failure_500 = { url: `${base}/failure/500`, success: true, confirmed_status: status, note: 'goto returns a response; status is inspectable, no throw on 500.', elapsed_seconds: Number(((Date.now() - t0) / 1000).toFixed(3)) };
    }

    // 6) Manual BFS crawl graph.
    {
      const t0 = Date.now();
      ...
      while (queue.length && seen.size < 30) {
        ...
        if (depth < 2) {
          ...
          for (const h of hrefs) { const abs = new URL(h, base).href.split('#')[0]; if (abs.startsWith(base) && !seen.has(abs)) queue.push({ url: abs, depth: depth + 1 }); }
        }
      }
      await page.close();
      ...
      summary.tests.local_crawl_graph = { url: base, success: true, pages_seen: pages.length, depth_counts: depthCounts, note: 'Puppeteer has no built-in crawl queue; BFS was hand-written.', elapsed_seconds: Number(((Date.now() - t0) / 1000).toFixed(3)) };
    }
    ...
    // 8) Public: Quotes JS (rendered).
    {
      ...
      try {
        ...
        summary.tests.public_quotes_js_rendered = { url: 'https://quotes.toscrape.com/js/', tested_on: now(), success: rows.length > 0, items: rows.length, note: 'JS-rendered quotes; Puppeteer renders them natively.', elapsed_seconds: Number(((Date.now() - t0) / 1000).toFixed(3)) };
    ...
    writeJson('puppeteer-test-summary.json', summary);
